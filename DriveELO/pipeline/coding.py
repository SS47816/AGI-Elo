...
def compute_match_results(df: pd.DataFrame, model_name: str) -> list:
    ...
    for _, row in df.iterrows():
        test_case_name = row["id"]
        test_case_label = row["expected_output"]
        accuracy = float(row["is_success"] == "True")
        score = accuracy
...
def main():
    ...
    parser.add_argument('--dataset_split', default="test", type=str, help=' "val", "test" ')
    parser.add_argument('--save_path', default="./data/coding/LiveCodeBench/",type=str, help='path to save processed data')
    ...
    compute_ratings(match_result_folder, rating_result_folder, dataset_name="LiveCodeBench", metric_name="Accuracy", left_margin=600)
