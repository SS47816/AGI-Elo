...
from DriveELO.utils.visualization import (plot_evaluation_trends,
...
def compute_ratings(match_result_folder: Path, rating_result_folder: Path, dataset_name: str, metric_name: str, dataset_folder: Path=None,
    ...
    summary_folder = Path("./data/summary")
    ...
    for i in tqdm(range(total_num_rounds), desc=f"Running Match Rounds"):
        ...
        model_df.to_csv(rating_result_folder.joinpath(f"rating_{dataset_name}_{metric_name}_model_{method}_{i + 1}.csv"))
        ...
        val_figure_path = summary_folder.joinpath(f"eval/{dataset_name}_{metric_name}_record_eval_{method}_{i + 1}.pdf")
        ...
        if record:
            ...
            eval_figure_path = summary_folder.joinpath(f"record_eval/{dataset_name}_{metric_name}_record_model_{method}_{i + 1}.pdf")
            ...
            model_figure_path = summary_folder.joinpath(f"record_model/{dataset_name}_{metric_name}_record_model_{method}_{i + 1}.pdf")
            plot_model_rating_trends(record_model_df, model_figure_path, dataset_name, show_plot=False)
            # Save Recorded Evaluation Results across % of matches

            record_eval_path = summary_folder.joinpath(f"record_eval/{dataset_name}_{metric_name}_record_eval_{method}_{i + 1}.csv")
            record_model_path = summary_folder.joinpath(f"record_model/{dataset_name}_{metric_name}_record_model_{method}_{i + 1}.csv")
            record_eval_df.to_csv(record_eval_path)
            record_model_df.to_csv(record_model_path)
