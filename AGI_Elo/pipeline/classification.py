...
def compute_match_results(df: pd.DataFrame, model_name: str) -> list:
    ...
    for _, row in df.iterrows():
        test_case_name = row["Image Filename"]
        test_case_label = row["True Label"]
        accuracy = int(row[model_name] == test_case_label)
        score = accuracy
...
def main():
    ...
    compute_ratings(match_result_folder, rating_result_folder, dataset_name="ImageNet", metric_name="Accuracy")
